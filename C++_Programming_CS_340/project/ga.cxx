...
namespace cs340 
{
  namespace 
  {
    ...
    struct schedule_compare 
    {
      ...
      /* TODO: WRITE CODE HERE DONE */
	  
    bool operator ()(schedule const& first , schedule const& second)
	  {
		  if(first.score(matrix_)  >  second.score(matrix_))
			  return true;
		  else
			  return false;
      ...
      size_t const pool_size, random_generator& gen)
    {
      ...
      vector<schedule>schedules_pool;
      schedules_pool.reserve(pool_size);
      ...
      /* TODO: WRITE CODE HERE DONE */
      uniform_int_distribution<size_t> dist(0,matrix.machines() - 1);
      ...
      /* TODO: WRITE CODE HERE DONE */
      generate_n(back_inserter(schedules_pool), pool_size, [&dist, &gen, &matrix](){
        schedule temp(matrix.tasks());
        for(size_t i = 0; i < temp.tasks(); i++){
          temp.set_task_assignment(i, dist(gen));
        }
        return temp;
      });
      ...
      /* TODO: WRITE CODE HERE DONE */
      stable_sort(schedules_pool.begin(), schedules_pool.end(), schedule_compare(matrix));

      // 5. Return the pool of schedules.

      /* TODO: WRITE CODE HERE DONE */
      return schedules_pool;
    }
    ...
    auto cross_over(schedule c1, schedule const& c2, random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE DONE */
      uniform_int_distribution<size_t> dist(0, c1.tasks() - 1);
      ...
      /* TODO: WRITE CODE HERE DONE */
      for(size_t i = crossover_point; i < c1.tasks(); i++){
        ...
        random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE DONE */
      uniform_int_distribution<size_t> task_dist(0, c.tasks() - 1);
      size_t random_task = task_dist(gen);
      
      uniform_int_distribution<size_t> machine_dist(0,matrix.machines() - 1);
      size_t random_machine = machine_dist(gen);
      
      c.set_task_assignment(random_task, random_machine);
    }
      ...
      vector<schedule>& gene_pool, random_generator& gen)
    {
      ...
      if ((x_pairs_count > 0) & (x_pairs_count < gene_pool.size())){
        ...
        /* TODO: WRITE CODE HERE DONE */
        auto erase_it = gene_pool.begin();
        erase_it += (gene_pool.size() - x_pairs_count);
        gene_pool.erase(erase_it,gene_pool.end());
        ...
        std::transform(begin(gene_pool), end(gene_pool), back_inserter(totals), [&matrix](auto const& iter){
          return iter.score(matrix);
        });
        ...
        /* TODO: WRITE CODE HERE DONE */
        uniform_real_distribution<double> roulette_style_dist{0, totals.back()};
        ...
        for(size_t i = 0; i < x_pairs_count; i++){
          ...
          /* TODO: WRITE CODE HERE DONE */
          double num_1 = roulette_style_dist(gen);
          double num_2 = roulette_style_dist(gen);
          ...
          /* TODO: WRITE CODE HERE DONE */
          auto it_low_1 = std::lower_bound(totals.begin(), totals.end(), num_1);
          ...
          /* TODO: WRITE CODE HERE DONE */
          auto offest_1 = std::distance(totals.begin(),it_low_1);
          
          // 2e iv. Perform the two above steps again. This time using the
          // second random number you drew.

          /* TODO: WRITE CODE HERE DONE */
          auto it_low_2 = std::lower_bound(totals.begin(), totals.end(), num_2);
          auto offest_2 = std::distance(totals.begin(),it_low_2);
          ...
          /* TODO: WRITE CODE HERE DONE */
          auto gene_iter_1 = gene_pool.begin() + offest_1;
          auto gene_iter_2 = gene_pool.begin() + offest_2;
          
          // 2e vi. Create a new schedule object by calling cross_over with
          // the two schedules pointed to by your computed iterators.

          /* TODO: WRITE CODE HERE DONE */
          schedule cross_over_result = cross_over(*gene_iter_1, *gene_iter_2, gen);
          ...
          /* TODO: WRITE CODE HERE DONE */
          auto it_gene_cross_over = std::lower_bound(gene_pool.begin(), gene_pool.end(), cross_over_result, schedule_compare(matrix));
          gene_pool.insert(it_gene_cross_over,cross_over_result);
      ...
      for (size_t j{}; j < num_mutations; ++j) 
      {
        ...
        /* TODO: WRITE CODE HERE DONE */
        auto pos = std::lower_bound(gene_pool.begin(), gene_pool.end(), *sample_solution, schedule_compare(matrix));
        ...
        if (pos <= sample_solution)
          std::rotate(pos,sample_solution,sample_solution+1);
        else
          std::rotate(sample_solution,sample_solution+1,pos);
      }
    }
      ...
      random_generator& gen)
  {
    ...
    /* TODO: WRITE CODE HERE DONE */
    vector <future<schedule>> future_winners;
    ...
    for (size_t i{}; i < args.threads; ++i) 
    {
      ...
      /* TODO: WRITE CODE HERE DONE */
      vector<size_t>seeds;
      generate_n(back_inserter(seeds), 6, [&dist, &gen](){
        return dist(gen);
      });
      ...
      future_winners.push_back(
        async(
          ...
          [&matrix, &args, seeds = move(seeds), i]() -> schedule 
          {
            ...
            /* TODO: WRITE CODE HERE DONE */
            auto thread_gene_pool = populate_gene_pool(matrix, pool_size, thread_gen);
            ...
            /* TODO: WRITE CODE HERE DONE */
            return run_simulation_n_times(matrix, thread_gene_pool, args.generations, thread_gen);
          }
        )
      );
    }
    ...
    /* TODO: WRITE CODE HERE DONE */
    for (size_t i = 0; i < future_winners.size(); i++){
      schedule&& current_future =  future_winners[i].get();
      if (schedule_compare(matrix)(current_future, best)){
        best = std::move(current_future);
      }
    }
